...
from skill_engineering.load_skill import load_skill, load_system_skill
...
class ArchitectureDesignerUnifiedSkill(Agent):
    def __init__(self):
        super().__init__("ArchitectureDesignerUnifiedSkill", model="claude-sonnet-4-20250514", system_prompt=load_system_skill("unified_system_instruction"))
    ...
    def refactor_adl(self, adl_name, new_requirement):
        ...
        """
        # Generate unified instruction for refactoring
        unified_user_instruction = load_skill("unified",
            adl_name=adl_name,
            new_requirement=new_requirement,
            task_type="refactor"
        )
        print(unified_user_instruction)
        refactoring_response = self.query(unified_user_instruction)
    ...
    def correct_adl(self, adl, liveness_properties):
        ...
        """
        # Generate unified instruction for validation correction
        unified_user_instruction = load_skill("unified",
            current_adl=adl,
            task_type="correct_validation"
        )
        correction_response = self.query(unified_user_instruction)
    ...
    def fix_syntax_rules_violation(self, adl):
        ...
        """
        # Generate unified instruction for syntax fixing
        unified_user_instruction = load_skill("unified",
            current_adl=adl,
            task_type="fix_syntax"
        )
        fixing_response = self.query(unified_user_instruction)
    ...
    def fix_misconfiguration(self, refactored_adl):
        ...
        while issues_1 or issues_2:
            if issues_1:
                # Generate unified instruction for misconfiguration 1 (will generate fresh issues inside)
                unified_user_instruction = load_skill("unified",
                    current_adl=refactored_adl,
                    task_type="misconfiguration_1"
                )
                misconfiguration_handling_response = self.query(unified_user_instruction)
                ...
                # Generate unified instruction for misconfiguration 2 (will generate fresh issues inside)
                unified_user_instruction = load_skill("unified",
                    current_adl=refactored_adl,
                    task_type="misconfiguration_2"
                )
                misconfiguration_handling_response = self.query(unified_user_instruction)
